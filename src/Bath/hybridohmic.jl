...
function integral_rate(sys, bath::HybridOhmicBath)
    ...
    for i in eachindex(sys.s)
        ...
        integrand_12 = (x)->(sys.b[i] * correlation(x, bath) + constant) * polaron_correlation(x, sys.a[i], bath) * exp(1.0im * sys.ω[i] * x)
        integrand_21 = (x)->(sys.b[i] * correlation(x, bath) + constant) * polaron_correlation(x, sys.a[i], bath) * exp(-1.0im * sys.ω[i] * x)
        push!(Γ10, integrate_1d(integrand_12, -Inf, Inf)[1])
        push!(Γ01, integrate_1d(integrand_21, -Inf, Inf)[1])
    end
    Γ10, Γ01
